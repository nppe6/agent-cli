...
function createJoinerTask(name, target = '.', options = {}) {
  ...
  if (!fs.existsSync(tasksDirectory) || !fs.statSync(tasksDirectory).isDirectory()) {
    throw new Error(`Missing ${SOURCE_DIRECTORY_NAME}/tasks directory. Run agent init first.`);
  }
...
- ${developer} has run \`agentos-cli agent developer init ${developer}\` or the equivalent runtime script.

...
这里存放统一源模式下的项目级本地 skills。

更新这个目录后执行 `agentos-cli agent sync`，内容会同步到当前项目已启用的 Agent 工具目录：

- `.claude/skills/`
- `.codex/skills/`

单工具模式同样会生成 `.agent-os/`，CLI 会在初始化和同步时把这些 skills 投影到所选工具目录。
...
统一源模式会优先导入到 `.agent-os/skills/`，导入后执行 `agentos-cli agent sync` 同步到已启用的 Agent 工具目录。

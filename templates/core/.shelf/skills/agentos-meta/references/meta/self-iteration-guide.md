...
Update to AgentOS Shelf itself → Update agentos-meta (meta skill)
```
...
# Copy template from agentos-meta/references/agentos-local-template.md
```
...
~/.claude/skills/
└── agentos-meta/                        # Shared meta-skill (vanilla AgentOS Shelf)
```
...
diff -r ~/.claude/skills/agentos-meta/references/ \
        ./new-agentos-meta/references/
```
...
cp -r ~/.claude/skills/agentos-meta ~/.claude/skills/agentos-meta.backup

# Update meta-skill
cp -r ./new-agentos-meta/* ~/.claude/skills/agentos-meta/
```
...
**Never** modify `agentos-meta` for project-specific changes.

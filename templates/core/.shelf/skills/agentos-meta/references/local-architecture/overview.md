...
`agentos-meta` is for user projects that have already run `agentos init`. The user's machine usually has only the npm-installed `agentos` command plus the AgentOS Shelf files generated inside the project; it may not have the AgentOS Shelf CLI source code.
...
4. **Put project-specific rules in `.shelf/spec/` or a local skill**: Do not put team conventions into `agentos-meta`.

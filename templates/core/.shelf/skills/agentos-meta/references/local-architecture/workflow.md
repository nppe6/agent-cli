...
- Platforms with sub-agent support: dispatch `agentos-implement` by default for implementation and `agentos-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `agentos-before-dev`, then executes directly.
...
- skills, such as `agentos-brainstorm` and `agentos-check`.

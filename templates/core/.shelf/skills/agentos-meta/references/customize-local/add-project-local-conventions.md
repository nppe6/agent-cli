...
Often the user does not need to change AgentOS Shelf mechanics; they need local AI to understand their team's conventions. In that case, prefer `.shelf/spec/` or a project-local skill instead of editing `agentos-meta`.
...
## Do Not Store Project-Private Rules In `agentos-meta`

`agentos-meta` is a public skill for understanding AgentOS Shelf architecture and local customization entry points. Put project-private content in:
...
This prevents future updates to AgentOS Shelf's built-in `agentos-meta` from overwriting the team's own conventions.

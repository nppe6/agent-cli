...
If the user wants only one platform to avoid sub-agents, first confirm whether that platform has a separate group in the workflow. Then change Phase 2 routing for that platform group instead of deleting all `agentos-implement` / `agentos-check` instructions across platforms.
...
| --- | --- | --- |
| `planning` | `prd.md` missing | Phase 1.1 (load `agentos-brainstorm`) |
...
| `in_progress` | implementation done, no `agentos-check` run | Phase 2.2 (`agentos-check`) |

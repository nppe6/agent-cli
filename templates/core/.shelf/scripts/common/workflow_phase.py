...
Extracts step-level content from .shelf/workflow.md and optionally filters
...
def get_phase_index() -> str:
    ...
    `## Customizing AgentOS Shelf (for forks)` (the docs-for-forks footer).
    ...
    for i, line in enumerate(lines):
        ...
        if start is None and stripped == _PHASE_INDEX_HEADING:
            start = i
            continue
        if start is not None and stripped == "## Customizing AgentOS Shelf (for forks)":
            end = i
            break

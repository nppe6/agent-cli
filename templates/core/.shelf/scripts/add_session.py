...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage .shelf/workspace and .shelf/tasks, then commit with a configured message."""
    commit_msg = get_session_commit_message(repo_root)
    add_result = subprocess.run(
        ["git", "add", "-A", ".shelf/workspace", ".shelf/tasks"],
        cwd=repo_root,
        capture_output=True,
        text=True,
    )
    if add_result.returncode != 0:
        ...
        print("[WARN] Please commit .shelf/ changes manually: git add .shelf && git commit", file=sys.stderr)
        return
    # Check if there are staged changes
    result = subprocess.run(
        ["git", "diff", "--cached", "--quiet", "--", ".shelf/workspace", ".shelf/tasks"],
        cwd=repo_root,
    )

...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.shelf/tasks/01-31-my-task')")
        return 1
    ...
    if not resolve_context_key():
        ...
        print(
            ...
            "or set SHELF_CONTEXT_ID before running task.py start."
        )
        return 1
...
Examples:
  ...
  python3 task.py create "Child task" --slug child --parent .shelf/tasks/01-21-parent
  python3 task.py add-context <dir> implement .shelf/spec/cli/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py start .shelf/tasks/01-21-add-login
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print("See .shelf/workflow.md Phase 1.3 or run:", file=sys.stderr)
        print(
            "  python3 ./.shelf/scripts/get_context.py --mode phase --step 1.3",
            file=sys.stderr,
        )
        print(
